// api/pensao/exportar.js
import { clienteDoRequest, json } from '../_core/supabase.js'
import { buscarMemoria } from './memoria.js'
import { pensaoParaDocx } from '../_core/docx.js'
import { pensaoParaXlsx } from '../_core/xlsx.js'
...
export async function montarExport({ supabase, casoId, versao, formato }) {
  ...
  const bytes = formato === 'xlsx'
    ? await pensaoParaXlsx(memoria, caso || {})
    : await pensaoParaDocx(memoria, caso || {})
  return {
    bytes,
    contentType: CT[formato],
    filename: `memoria-pensao-v${memoria.versao}.${formato}`,
  }
}
